...
//	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);		//将JTAG引脚失能，作为普通GPIO引脚使用
	...
	/*配置时钟源*/
	TIM_InternalClockConfig(TIM8);		//选择TIM2为内部时钟，若不调用此函数，TIM默认也为内部时钟
	...
	TIM_OC2Init(TIM8, &TIM_OCInitStructure);						//将结构体变量交给TIM_OC1Init，配置TIM2的输出比较通道1
	//TIM_OC2Init(TIM8, &TIM_OCInitStructure);
	/*TIM使能*/
	TIM_Cmd(TIM8, ENABLE);			//使能TIM2，定时器开始运行
	TIM_CtrlPWMOutputs(TIM8, ENABLE); //通用定时器不用加
}
